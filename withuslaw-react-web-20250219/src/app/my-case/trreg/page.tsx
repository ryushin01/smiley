...
import { ExclamationMarkWithGrayBg, ExclamationMarkBlue } from "@icons";
...
export default function MY_TR_001_M() {
  ...
  const [isSoleOwnership, setIsSoleOwnership] = useState(false);
  ...
  const { mutate: checkTrreg } = useMutation({
    ...
    onSuccess(data, variables, context) {
      if (data.available === true) {
        ...
        // 번호 불일치시 토스트팝업 노출
        callToast({
          msg: `접수번호 ${data.rgstrNo}가 일치하지 않습니다.`,
          status: "error",
          dim: true,
        });
      }
  ...
  console.log("isbefDbsmtCnclYn", isbefDbsmtCnclCd);
  console.log("isRequiredErNums", isRequiredErNums);
  // const isSoleOwnership = !!initRegInfo?.buyerInfoList && initRegInfo?.buyerInfoList.length === 1;
  ...
  const handleSearchResultClick = (
    ...
    /* TODO : 등기고유번호(uniqNo), 등기소 코드 주석 해제*/
    // setValue("regoCd", regoCd); // 등기소 코드
    // setValue("uniqNo", uniqNo); // 등기고유번호
    close(); // 검색창 닫기
  };

  const onSubmit = (data: TForm) => {
    console.log("data", data);
    // console.log("data", data.regoCd);
    // console.log("data", data.uniqNo);

    /* TODO : setPayload 주석 해제 및 onpenConfirm 제거*/
    openConfirm();

    // 이전건일경우 이전접수번호 수만큼 리스트 생성
    // if (regType === RGSTR_GB_CD["01"]) {
    //   const checkList: TCheckBody[] = bfNumFields.map((bfNum) => ({
    //     rgstrNo: bfNum.rgstrNo, // 등기접수번호
    //     regoCd: data.regoCd!, // 등기소코드
    //     loanNo, // 여신번호
    //     tempUniqNo: data.uniqNo!, // 등기고유번호
    //   }));

    //   setPayload({ checkList });
    // }
    // 설정건일경우 설정접수번호 수만큼 리스트 생성
    // if (regType === RGSTR_GB_CD["02"]) {
    //   const checkList: TCheckBody[] = esNumFields.map((esNum) => ({
    ...
    //   });
    // }
    // payload && checkTrreg(payload.checkList);

    if (regType === RGSTR_GB_CD["01"]) {
      return setForm({
        regoNm: data.regoNm,
        uptUnqNo: data.uptUnqNo!,
        loanNo,
    ...
    if (regType === RGSTR_GB_CD["02"]) {
      console.log(data.erNums?.[0].rgstrNo);
      console.log("regoNm", data.regoNm);
      return setForm({
        regoNm: data.regoNm,
        uptUnqNo: data.uptUnqNo!,
        loanNo,
  ...
  useEffect(() => {
    ...
    console.log("IsSoleOwnership : " + isSoleOwnership);
    console.log(
      "initRegInfo?.rgstrInfoList.length : " + initRegInfo?.rgstrInfoList.length
    );
      ...
      >
        ...
        <div ref={expandedRef}>
          ...
          {regType === RGSTR_GB_CD["02"] &&
            esNumFields.map((bfNum, i) => (
              <Input.InputContainer
                className={`px-4 py-6 ${
                  i !== esNumFields.length - 1 &&
                  "border border-b-1 border-kos-gray-200"
                } `}
                key={`${i}${bfNum.byrBirthDt}`}
              >
                <Input.Label
                  htmlFor={"trregNo"}
                  required={true}
